...
cmd_vmnet_usage() {
  echo_message "Usage: $(basename "$0") vmnet <subcommand> [options/arguments]"
  echo_message "\nSubcommands:"
  ...
  echo_message "\nExamples:"
  echo_message "  $(basename "$0") vmnet create --name myvmnet0 --ip 192.168.1.1/24"
  echo_message "  $(basename "$0") vmnet list"
  echo_message "  $(basename "$0") vmnet destroy myvmnet0"
}
