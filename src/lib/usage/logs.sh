...
cmd_logs_usage() {
  echo_message "Usage: $(basename "$0") logs <vm_name> [--tail <number_of_lines>] [-f]"
  echo_message "\nDescription:"
  ...
  echo_message "\nExamples:"
  echo_message "  $(basename "$0") logs my_vm                 # Show last 100 lines of my_vm's log"
  echo_message "  $(basename "$0") logs my_vm --tail 50       # Show last 50 lines of my_vm's log"
  echo_message "  $(basename "$0") logs my_vm -f              # Follow my_vm's log in real-time"
}